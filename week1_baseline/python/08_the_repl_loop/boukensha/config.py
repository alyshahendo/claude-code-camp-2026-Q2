...
class Config:
    ...
    # Default prompts shipped alongside this step. Mirrors the Ruby reference,
    # which resolves one level above the step root; the player task overrides
    # the system prompt from .boukensha, so this default is only a fallback.
    ...
    def _resolve_dir(self):
        # 1. Explicit override
        env = os.environ.get("BOUKENSHA_DIR")
        if env:
