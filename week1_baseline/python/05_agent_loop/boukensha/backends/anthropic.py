...
class Anthropic(Base):
    """Serializes context into the Anthropic ``/v1/messages`` format."""
    ...
    def to_messages(self, messages):
        ...
        for msg in messages:
            if msg.role == "tool_result":
                ...
                serialized.append({"role": str(msg.role), "content": msg.content})
        return serialized
    ...
    # Anthropic's content array already IS the normalized shape, so this is a
    # thin pass-through and no assistant_message rebuild is needed.
